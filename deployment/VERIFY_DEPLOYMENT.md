...
nslookup tms-chat-staging.hotelsogo-ai.com
...
# Open in browser:
https://tms-chat-staging.hotelsogo-ai.com/docs
```
...
```bash
# 1. Check DNS resolution
nslookup tms-chat.hotelsogo-ai.com
# Should return: 47.80.71.165

# 2. Check HTTPS is working
curl -I https://tms-chat.hotelsogo-ai.com
# Should return: HTTP/2 200

# 3. Check backend health
curl https://tms-chat.hotelsogo-ai.com/health

# 4. Check API health
curl https://tms-chat.hotelsogo-ai.com/api/v1/health
...
- [ ] **DNS Resolution**
  ```bash
  dig tms-chat-staging.hotelsogo-ai.com +short
  # Should show: 47.80.66.95
  ```

- [ ] **SSL Certificate**
  ```bash
  openssl s_client -connect tms-chat-staging.hotelsogo-ai.com:443 -servername tms-chat-staging.hotelsogo-ai.com < /dev/null 2>/dev/null | grep -A2 'Certificate chain'
  # Should show Let's Encrypt certificate
  ```

- [ ] **Port Accessibility**
  ```bash
  nc -zv tms-chat-staging.hotelsogo-ai.com 443
...
- [ ] **API endpoints working**
  ...
  curl https://tms-chat-staging.hotelsogo-ai.com/docs
  ```
...
- [ ] **Homepage accessible**
  ```bash
  curl -L https://tms-chat-staging.hotelsogo-ai.com
  # Should return HTML
  ```
...
- [ ] **Database accessible from ECS**
  ```bash
  ssh root@47.80.66.95 "psql -h pgm-5tsq1t17984pyydh0o.pgsql.ap-southeast-6.rds.aliyuncs.com -U postgres -d tms_staging_db -c 'SELECT version();'"
  # Should return PostgreSQL version
  ```

- [ ] **Database tables exist**
  ```bash
  ssh root@47.80.66.95 "psql -h pgm-5tsq1t17984pyydh0o.pgsql.ap-southeast-6.rds.aliyuncs.com -U postgres -d tms_staging_db -c '\dt'"
...
- [ ] **Redis accessible from ECS**
  ```bash
  ssh root@47.80.66.95 "redis-cli -h r-5tsm9aoc5soeozbq3v.redis.ap-southeast-6.rds.aliyuncs.com -p 6379 -a gosgoj-9gygwo-Dusbob PING"
  # Should return: PONG
  ```

- [ ] **Redis can set/get**
  ```bash
  ssh root@47.80.66.95 "redis-cli -h r-5tsm9aoc5soeozbq3v.redis.ap-southeast-6.rds.aliyuncs.com -p 6379 -a gosgoj-9gygwo-Dusbob SET test 'hello'"
  ssh root@47.80.66.95 "redis-cli -h r-5tsm9aoc5soeozbq3v.redis.ap-southeast-6.rds.aliyuncs.com -p 6379 -a gosgoj-9gygwo-Dusbob GET test"
  # Should return: hello
  ```
...
#### Authentication
- [ ] Navigate to: `https://tms-chat-staging.hotelsogo-ai.com`
...
openssl s_client -connect tms-chat-staging.hotelsogo-ai.com:443 -servername tms-chat-staging.hotelsogo-ai.com < /dev/null 2>/dev/null | openssl x509 -noout -dates
```
...
ssh root@47.80.66.95 "psql -h pgm-5tsq1t17984pyydh0o.pgsql.ap-southeast-6.rds.aliyuncs.com -U postgres -d tms_staging_db -c 'SELECT COUNT(*) FROM pg_stat_activity;'"
```
...
wscat -c wss://tms-chat-staging.hotelsogo-ai.com/ws
...
curl -w "@-" -o /dev/null -s https://tms-chat-staging.hotelsogo-ai.com <<'EOF'
...
- `https://tms-chat-staging.hotelsogo-ai.com/api/v1/health`
- `https://tms-chat-staging.hotelsogo-ai.com/docs` (staging)
